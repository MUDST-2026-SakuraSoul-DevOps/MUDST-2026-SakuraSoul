...
describe('LogoutConfirmModal', () => {
  ...
  it('closes the modal when clicking Cancle', async () => {
    const user = userEvent.setup()
    render(<LogoutConfirmModal />)

    // Test 2: Verify that clicking Cancle closes the modal without logging out.
    await user.click(screen.getByRole('button', { name: 'ออกจากระบบ' }))
    await user.click(screen.getByRole('button', { name: /cancle/i }))
