...
### ข้อมูลผู้เช่าในฟอร์มอื่น (SSK-136, 25 ก.ย. 2569)

ผู้เช่าเก็บแค่ห้าช่องในตารางบน (`fullName`, `nationalId`, `lineId`, `phone`, `email`)
หน้าเว็บเคยมีช่องที่ดูเหมือนบันทึกได้แต่ไม่มีที่เก็บจริง จึงเก็บกวาดตามนี้

| ที่ | เดิม | ตอนนี้ |
| --- | --- | --- |
| ฟอร์ม Add / Edit Tenant | มีช่อง Lease Period กับ Room Type ที่ backend มองข้าม (บั๊ก SSK-136) และ Add สร้างอีเมล `<ชื่อ>@example.com` เองลงฐาน | ตัดสองช่องนั้นออก สัญญาสร้างที่หน้า Contracts หรือ check-in บน Dashboard มีช่อง **Email ไม่บังคับ** แทน ไม่กรอกก็ไม่ส่ง |
| ป็อปอัปห้องบน Dashboard (Occupied / check-in) | Phone, National ID, Line ID เป็นค่าปลอมเหมือนกันทุกคน มีช่อง Emergency Contact | แสดงอย่างเดียวจากผู้เช่าจริง (`Not provided` ถ้าว่าง) ตัด Emergency Contact เพราะไม่มีช่องนี้ที่ไหนในระบบ |
| ฟอร์ม Create / Edit Contract | ช่อง ID / Phone / Line ID แก้ได้แต่ไม่ถูกส่ง เติมค่าปลอมไว้ | แสดงอย่างเดียวจากผู้เช่าที่เลือก แก้ได้ที่หน้า Tenants |
| หน้า Tenants | ป้ายสถานะ Pending / Overdue ตั้งตาม id ของผู้เช่า | ป้ายตามสัญญาจริง Active / Ending Soon / Ended ส่วน Pending / Overdue ต้องมาจากใบเสร็จ (SSK-16) |

งานอีเมลไม่บังคับที่ยังค้างในตาราง SSK-99 ข้างบน (`types.ts`, `domain/tenant.ts`, backend จำลอง
และฟอร์ม) ปิดครบในรอบนี้ ผู้เช่าที่ไม่มีอีเมล (เช่น Kenji Watanabe ใน `DevDataSeeder`) ต้องไม่ทำให้
หน้าไหนพัง เดิมช่องค้นหาหน้า Tenants เรียก `email.toLowerCase()` ตรง ๆ แล้วพังบน backend จริง
...
**ค่าส่วนกลางในฟอร์ม Create Contract (SSK-136)** ค่าตั้งต้นมาจาก `commonAreaFee` ของ Config
และส่งไปใน `LeaseRequest.commonAreaFee` จริง แอดมินแก้เป็นรายสัญญาได้ เว้นว่างแล้ว backend
คัดลอกจาก Config เอง ตอนแก้สัญญาแสดงค่าที่ล็อกไว้อย่างเดียว เหมือนค่าเช่า
(เดิมช่องนี้ตั้งตายตัวไว้ 200 ป้ายบอกว่ามาจาก Config แต่ไม่เคยถูกส่ง)
...
- **ไม่มี `POST /api/rooms` และรอบนี้จะไม่ทำ** ห้องเป็นชุดตายตัว 24 ห้องจาก `V2__seed_rooms.sql`
  ทีมถอดปุ่ม Add Unit ออกไปแล้ว (branch `bugfix/units/remove-add-unit-button`)
  และ SSK-140 ถอด `createRoom`, `CreateRoomRequest`, `validateRoom` กับ route `POST /api/rooms`
  ของ backend จำลองออกจากหน้าเว็บด้วย ให้ backend จำลองมีเฉพาะ endpoint ที่ Spring มีจริง
  ถ้าจะเอากลับมา ต้องแก้เอกสารนี้ก่อนแล้วทำพร้อมกันทั้งสองฝั่ง
- **`RoomDetail` ไม่มี `address`** ฟิลด์นี้เคยมีแต่ในหน้าเว็บตามฟอร์ม Add Unit ของ Figma
  backend ไม่เคยส่งมา ดูเหตุผลในข้อ "เอกสารสัญญาไม่พิมพ์ที่อยู่ของตึก" หัวข้อถัดไป

**`PUT /api/tenants/{id}`** รับ body เดียวกับตอนเพิ่ม กฎทุกข้อเหมือนกัน รวมถึงเลขบัตร**บังคับ**
เลขบัตรเดิมของตัวเองไม่นับว่าซ้ำ ช่องของสัญญา (`startDate`, `endDate`, `roomType`) ไม่ใช่ข้อมูลผู้เช่า
ถ้าส่งมาจะถูกมองข้าม ฟอร์ม Add / Edit Tenant เลิกส่งแล้ว (SSK-136)

**PUT แทนทั้งก้อน** ช่องไม่บังคับที่ไม่ส่งมา (`email`, `lineId`) ถูกเก็บเป็น `null` ไม่ใช่คงค่าเดิม
ฟอร์ม Edit Tenant จึงต้องส่งอีเมลเดิมกลับไปทุกครั้ง เดิมไม่ส่ง แก้ผู้เช่าบน backend จริงแล้วอีเมลหาย
(backend จำลองเคยคงค่าเดิมไว้จึงไม่มีใครเห็น ตอนนี้ทำตัวเหมือน `TenantService.update` แล้ว)
...
- **เอกสารสัญญาไม่พิมพ์ที่อยู่ของตึก** (ตัดสินใจ 26 ก.ย. 2569 ใน SSK-140)
  Figma มีแถว Address ในหัวข้อ Property Details แต่สัญญา API ไม่เคยมี `address` ทั้งในห้องและใน
  `apartment_config` และ PDF ฝั่ง backend (`lease-contract.html`) ก็ไม่มีบรรทัดที่อยู่
  หน้าเว็บเคยโชว์ `Building A, 123 Street` ซึ่งเป็นค่าตัวอย่างของ backend จำลอง บน backend จริง
  Preview จึงขึ้น Not available ส่วน PDF ที่ดาวน์โหลดพิมพ์ค่าตัวอย่างนั้นลงเอกสารที่ผู้เช่าเซ็น
  เลยตัดแถวนี้ออกจาก Contract Preview, PDF ในเบราว์เซอร์ และ Contract Template แทนการพิมพ์ค่าปลอม
  ระบบดูแลตึกเดียว เอกสารระบุคู่สัญญาเป็น Sakura Soul พร้อมเลขห้องกับชั้นอยู่แล้ว
  ถ้าวันหลังต้องมีที่อยู่จริง ให้เก็บเป็น**ที่อยู่ค่าเดียวของตึก** ไม่ใช่คอลัมน์รายห้อง
  เพราะ 24 แถวจะเก็บค่าเดียวกันซ้ำแล้วเพี้ยนกันได้ เป็นปัญหาแหล่งความจริงสองที่แบบเดียวกับที่
  อาจารย์จับได้เรื่องค่าเช่า (ดูคอมเมนต์ใน `V12__rent_by_room_type.sql`) ที่ที่น่าจะใช้คือ
  `apartment_config` แต่ US-16 นิยามตารางนั้นไว้เป็นอัตราค่าบริการ จึงต้องตกลงกันก่อนว่าจะขยายนิยาม
  หรือแยกตารางข้อมูลตึก แก้เอกสารนี้ก่อนแล้วทำทั้งสองฝั่งพร้อมกัน
